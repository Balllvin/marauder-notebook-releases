...
The five-minute schedule remains the normal publisher trigger. A lock-aware intake is eligible only when its versioned branch points to the same commit as `publication-lock/notebook`; the publisher rechecks both refs immediately before publication. Legacy branch selection remains available only while the lock is absent, which makes migration fail closed if a lock appears during an older run. If GitHub disables scheduled workflows after inactivity, a repository administrator can manually dispatch `Notebook release publisher` on `main`. The workflow rejects every other ref, checks out `main` explicitly, and proves it is running the current protected commit before touching intake.
