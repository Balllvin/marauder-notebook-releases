...
class VerifyAppBundleTests(unittest.TestCase):
    ...
    def test_nested_code_requires_the_exact_producer_entitlements(self) -> None:
        app = self.root / "Marauder Notebook.app"
        main = app / "Contents" / "MacOS" / verify_app_bundle.EXECUTABLE_NAME
        helper = (
            app
            / "Contents"
            / "Frameworks"
        ...
        autoupdate = helper.with_name("Autoupdate")
        ...
        verify_app_bundle.verify_nested_code_entitlements(
            app,
            [main, autoupdate],
            entitlement_reader=lambda *_: autoupdate_entitlements,
        )
        with self.assertRaisesRegex(
            verify_app_bundle.BundleVerificationError,
            "unexpected signed entitlements",
